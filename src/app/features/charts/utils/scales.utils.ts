...
import { HGrid, HDateTime, HTime, Kind, HBool } from 'haystack-core';
...
export function generateTimeUnit(
  ...
  const TIME_DIFFERENCE_HOURS = DateTime.fromISO(SECOND_TIMESTAMP).diff(
    DateTime.fromISO(FIRST_TIMESTAMP!),
    'hours'
  ).hours;
